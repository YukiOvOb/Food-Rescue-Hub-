...
public class Co2AnalyticsService {
  ...
  public Co2SummaryDto getCo2Summary(Long supplierId, int days) {
    ...
    List<Co2CategoryBreakdownDto> breakdown =
        orderRepository.findCo2BreakdownBySupplierAndStatusSince(supplierId, "COMPLETED", from);
  ...
  public byte[] generateCo2ReportPdf(Co2SummaryDto summary, Long supplierId) {
    ...
    document.add(
        new Paragraph(
            "Period: " + summary.getFrom().format(fmt) + " to " + summary.getTo().format(fmt),
            bodyFont));
    document.add(new Paragraph(" ", bodyFont));

    document.add(new Paragraph("Summary", headerFont));
    document.add(
        new Paragraph("Total CO2 saved (kg): " + format(summary.getTotalCo2Kg()), bodyFont));
    document.add(
        new Paragraph("Total weight (kg): " + format(summary.getTotalWeightKg()), bodyFont));
    document.add(new Paragraph(" ", bodyFont));
