 ...
 *
 * <p>For user story 7 – accept/reject a pending order. For user story 13 – inventory is decremented
 * on accept (delegated to {@link InventoryService}).
 */
@Service
...
public class OrderService {
  ...
  public Order createOrderFromCart(
      Long consumerId, LocalDateTime pickupSlotStart, LocalDateTime pickupSlotEnd) {
    Cart cart =
        cartRepository
            .findFirstByConsumer_ConsumerIdAndStatusOrderByCreatedAtDesc(consumerId, "ACTIVE")
            .orElseThrow(
                () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "No active cart found"));
    ...
    if (pickupSlotStart == null
        || pickupSlotEnd == null
        || !pickupSlotStart.isBefore(pickupSlotEnd)) {
    ...
    for (CartItem item : sortedItems) {
      ...
      orderItem.setLineTotal(
          listing.getRescuePrice().multiply(BigDecimal.valueOf(item.getQuantity())));
    ...
    log.info(
        "Order created from cart with ID: {} and pickup token generated", savedOrder.getOrderId());
    return savedOrder;
  }
  ...
  /**
   * 1. Validates listing exists and is ACTIVE. 2. Performs a fast (unlocked) stock check – fails
   * early with a friendly message. 3. Builds an Order in PENDING state and persists it.
   *
   * <p>The real oversell guard fires later when the supplier clicks ACCEPT (inside {@link
   * #acceptOrder}), but checking here avoids letting obviously-impossible orders into the queue at
   * all.
   */
  @Transactional
  public Order createOrder(CreateOrderRequest req) {
    Listing listing =
        listingRepository
            .findById(req.getListingId())
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Listing not found with id: " + req.getListingId()));

    if (!"ACTIVE".equalsIgnoreCase(listing.getStatus())) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Listing " + req.getListingId() + " is not active");
    }

    ConsumerProfile consumer =
        consumerProfileRepository
            .findById(req.getConsumerId())
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND,
                        "Consumer not found with id: " + req.getConsumerId()));
  ...
  /**
   * Inventory is NOT touched here – it was never decremented for a PENDING order. If we later add a
   * "cancel an ACCEPTED order" flow, call {@link InventoryService#restoreStock} inside that method.
   */
  @Transactional
  ...
  /**
   * Returns the supplier's order queue for a specific store. {@code status} is optional – pass
   * {@code null} for all statuses.
   */
  ...
  public List<com.frh.backend.dto.TopSellingItemDto> getTopSellingItems(
      Long supplierId, String status, int limit) {
    return orderRepository.findTopSellingItemsBySupplierAndStatus(
        supplierId, status, PageRequest.of(0, Math.max(1, limit)));
  }
}
