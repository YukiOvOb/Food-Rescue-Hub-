...
public class ListingReviewService {
    ...
    public ListingReviewResponse createReview(Long consumerId, CreateListingReviewRequest request) {
        ...
        if (!REVIEWABLE_STATUSES.contains(status)) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Order must be completed before submitting a review");
        }

        if (order.getOrderItems() == null || order.getOrderItems().isEmpty()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Order does not contain any listings");
        }

        Listing listing = order.getOrderItems().stream()
            .map(item -> item.getListing())
            .filter(itemListing -> itemListing != null && request.getListingId().equals(itemListing.getListingId()))
            .findFirst()
            .orElseThrow(() -> new ResponseStatusException(
                HttpStatus.BAD_REQUEST,
                "Listing is not part of the specified order"
            ));
        ...
        if (listingReviewRepository.existsByOrder_OrderIdAndListing_ListingIdAndConsumer_ConsumerId(
            ...
            throw new ResponseStatusException(HttpStatus.CONFLICT, "You have already reviewed this listing for the order");
        }
    ...
    public List<ListingReviewResponse> getReviewsByListing(Long listingId) {
        return listingReviewRepository.findByListing_ListingIdOrderByCreatedAtDesc(listingId)
            .stream()
            .map(this::toResponse)
            .toList();
    }
    ...
    public void deleteReview(Long reviewId, Long consumerId) {
        ListingReview review = listingReviewRepository.findById(reviewId)
