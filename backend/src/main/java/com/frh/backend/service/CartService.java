...
public class CartService {
     ...
     * a new one is created and returned as a CartResponseDto.
     *
    ...
    /**
     * Removes an item from the cart and returns the updated cart as a CartResponseDto.
     * If the cart becomes empty, the store reference is cleared.
     * 
     * @param session the HTTP session containing the user ID
    ...
    private Cart getOrCreateActiveCartEntity(HttpSession session) {
        ...
        return cartRepository.findFirstByConsumer_ConsumerIdAndStatusOrderByCreatedAtDesc(consumerId, "ACTIVE")
                .orElseGet(() -> {
    ...
    /**
     * Maps a Cart entity to a CartResponseDto for API responses.
     * Calculates subtotal and total from all cart items.
     * 
        ...
        BigDecimal totalSavings = itemDtos.stream()
            .map(i -> {
                if (i.getOriginalPrice() != null && i.getUnitPrice() != null) {
                    return i.getOriginalPrice().subtract(i.getUnitPrice()).multiply(BigDecimal.valueOf(i.getQty()));
                } else {
    ...
    /**
     * Maps a CartItem entity to a CartResponseDto.CartItemDto.
     * Extracts the first photo from the listing's photos if available.
     * 
    ...
    public CartResponseDto.CartItemDto toCartItemDto(CartItem item) {
        ...
        dto.setLineTotal(item.getListing().getRescuePrice().multiply(BigDecimal.valueOf(item.getQuantity())));
        ...
        dto.setPickupStart(item.getListing().getPickupStart() != null ? item.getListing().getPickupStart().toString() : null);
        dto.setPickupEnd(item.getListing().getPickupEnd() != null ? item.getListing().getPickupEnd().toString() : null);
        dto.setStoreName(item.getListing().getStore() != null ? item.getListing().getStore().getStoreName() : null);

        // Add originalPrice and savingsLabel
        dto.setOriginalPrice(item.getListing().getOriginalPrice());
        BigDecimal savingsAmount = item.getListing().getOriginalPrice().subtract(item.getListing().getRescuePrice());
        dto.setSavingsLabel(savingsAmount.compareTo(BigDecimal.ZERO) > 0 ? "Worth $" + item.getListing().getOriginalPrice().intValue() + "+" : null);
