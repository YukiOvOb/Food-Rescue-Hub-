...
/** Service for managing user interactions Records VIEW, CLICK, SEARCH, and ADD_TO_CART events */
@Service
...
public class InteractionService {
  ...
  public UserInteraction recordInteraction(UserInteractionRequest request) {
    ...
    ConsumerProfile consumer =
        consumerProfileRepository
            .findById(request.getConsumerId())
            .orElseThrow(
                () -> new RuntimeException("Consumer not found: " + request.getConsumerId()));
    ...
    Listing listing =
        listingRepository
            ...
            .orElseThrow(
                () -> new RuntimeException("Listing not found: " + request.getListingId()));
  ...
  private void updateStatistics(
      Long consumerId, Long listingId, UserInteraction.InteractionType type) {
    try {
          ...
          consumerStatsRepository
              ...
              .orElseGet(
                  () -> {
                    ...
                    ConsumerProfile consumer =
                        consumerProfileRepository.findById(consumerId).orElse(null);
