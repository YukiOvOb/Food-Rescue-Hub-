...
public interface StoreRatingRepository extends JpaRepository<StoreRating, Long> {
  ...
  @Query("SELECT AVG(sr.rating) FROM StoreRating sr " + "WHERE sr.store.storeId = :storeId")
  BigDecimal calculateAvgRating(@Param("storeId") Long storeId);

  // Count ratings for store
  @Query("SELECT COUNT(sr) FROM StoreRating sr " + "WHERE sr.store.storeId = :storeId")
  ...
  @Query(
      "SELECT sr.store.storeId, AVG(sr.rating) as avgRating, COUNT(sr) as ratingCount "
          + "FROM StoreRating sr "
  ...
  @Query(
      "SELECT COUNT(sr) > 0 FROM StoreRating sr "
          + "WHERE sr.consumer.consumerId = :consumerId "
          + "AND sr.store.storeId = :storeId")
  Boolean hasConsumerRatedStore(
      @Param("consumerId") Long consumerId, @Param("storeId") Long storeId);
}
