...
public interface SearchLogRepository extends JpaRepository<SearchLog, Long> {
  ...
  @Query(
      "SELECT sl.queryText, COUNT(sl) as searchCount "
          + "FROM SearchLog sl "
  ...
  @Query(
      "SELECT sl FROM SearchLog sl " + "WHERE sl.resultsCount = 0 " + "ORDER BY sl.createdAt DESC")
  ...
  @Query("SELECT AVG(sl.clickPosition) FROM SearchLog sl " + "WHERE sl.clickPosition IS NOT NULL")
