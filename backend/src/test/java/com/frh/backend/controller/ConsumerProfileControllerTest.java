...
class ConsumerProfileControllerTest {
    ...
    void updateLocation_success() throws Exception {
        ...
        Mockito.when(consumerProfileRepository.findById(1L))
                .thenReturn(Optional.of(consumer));

        Mockito.when(consumerProfileRepository.save(Mockito.any()))
                .thenReturn(consumer);
        ...
        mockMvc.perform(put("/api/consumer/profile/{consumerId}/location", 1L)
    ...
    void updateLocation_notFound() throws Exception {
        ...
        Mockito.when(consumerProfileRepository.findById(99L))
        ...
        mockMvc.perform(put("/api/consumer/profile/{consumerId}/location", 99L)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
    ...
    void getLocation_success() throws Exception {
        ...
        Mockito.when(consumerProfileRepository.findById(2L))
                .thenReturn(Optional.of(consumer));

        mockMvc.perform(get("/api/consumer/profile/{consumerId}/location", 2L))
                .andExpect(status().isOk())
    ...
    void getLocation_notFound() throws Exception {
        ...
        Mockito.when(consumerProfileRepository.findById(88L))
                .thenReturn(Optional.empty());

        mockMvc.perform(get("/api/consumer/profile/{consumerId}/location", 88L))
                .andExpect(status().isNotFound());
    }
}
