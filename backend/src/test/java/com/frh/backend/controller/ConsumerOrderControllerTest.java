...
class ConsumerOrderControllerTest {
  ...
  void getOrdersByConsumerId_success() throws Exception {
    ...
    Mockito.when(consumerOrderService.getOrdersByConsumerId(1L)).thenReturn(List.of(order));

    mockMvc
        .perform(get("/api/consumer/orders").sessionAttr("USER_ID", 1L))
  ...
  void getOrdersByConsumerId_unauthorized() throws Exception {
    ...
    mockMvc.perform(get("/api/consumer/orders")).andExpect(status().isUnauthorized());
  }
  ...
  void getOrderById_found() throws Exception {
    ...
    Mockito.when(consumerOrderService.getOrderById(10L)).thenReturn(Optional.of(order));

    mockMvc
        .perform(get("/api/consumer/orders/order/{orderId}", 10L).sessionAttr("USER_ID", 1L))
        .andExpect(status().isOk());
  }
  ...
  void getOrderById_unauthorized() throws Exception {
    ...
    mockMvc
        .perform(get("/api/consumer/orders/order/{orderId}", 10L))
  ...
  void getOrderById_notFound() throws Exception {
    ...
    Mockito.when(consumerOrderService.getOrderById(99L)).thenReturn(Optional.empty());

    mockMvc
        .perform(get("/api/consumer/orders/order/{orderId}", 99L).sessionAttr("USER_ID", 1L))
  ...
  void getOrdersByConsumerIdAndStatus_success() throws Exception {
    ...
    Mockito.when(consumerOrderService.getOrdersByConsumerIdAndStatus(1L, "COMPLETED"))
        .thenReturn(List.of(order));

    mockMvc
        .perform(
            get("/api/consumer/orders/status/{status}", "COMPLETED").sessionAttr("USER_ID", 1L))
        .andExpect(status().isOk());
  }
  ...
  void getOrdersByConsumerIdAndStatus_unauthorized() throws Exception {
    ...
    mockMvc
        .perform(get("/api/consumer/orders/status/{status}", "COMPLETED"))
        .andExpect(status().isUnauthorized());
  }
  ...
  void updateOrderStatus_success() throws Exception {
    ...
    Mockito.when(consumerOrderService.getOrderById(10L)).thenReturn(Optional.of(existingOrder));
    Mockito.when(consumerOrderService.updateOrderStatus(10L, "CANCELLED")).thenReturn(updatedOrder);

    mockMvc
        .perform(
            patch("/api/consumer/orders/order/{orderId}/status", 10L)
  ...
  void updateOrderStatus_unauthorized() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/consumer/orders/order/{orderId}/status", 10L).param("status", "CANCELLED"))
        .andExpect(status().isUnauthorized());
  }
  ...
  void updateOrderStatus_notFound() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/consumer/orders/order/{orderId}/status", 99L)
                .sessionAttr("USER_ID", 1L)
                .param("status", "CANCELLED"))
        .andExpect(status().isNotFound());
  }
