...
class PickupTokenControllerTest {
    ...
    void generateQRCode_saveFailure() throws Exception {
        ...
        Mockito.when(pickupTokenRepository.findByOrderId(3L))
                .thenReturn(Optional.empty());
        ...
        try (MockedStatic<com.frh.backend.util.QRCodeGenerator> mocked =
            ...
            mockMvc.perform(post("/api/pickup-tokens/{orderId}/generate-qrcode", 3L))
                    .andExpect(status().isInternalServerError());
        }
    }
    ...
    void getPickupToken_zeroOrderId() throws Exception {
        ...
        Mockito.when(pickupTokenRepository.findByOrderId(0L))
                .thenReturn(Optional.empty());

        mockMvc.perform(get("/api/pickup-tokens/{orderId}", 0L))
                .andExpect(status().isNotFound());
    }
    ...
    void generateQRCode_tokenExistsPropertyCheck() throws Exception {
        ...
        Mockito.when(pickupTokenRepository.findByOrderId(4L))
                .thenReturn(Optional.of(token));

        try (MockedStatic<com.frh.backend.util.QRCodeGenerator> mocked =
                     Mockito.mockStatic(com.frh.backend.util.QRCodeGenerator.class)) {

            mocked.when(() -> generateQRCode(Mockito.eq("QR-EXISTING"), Mockito.any()))
                    .thenReturn("/qrcode/order_4_QR-EXISTING.png");

            mockMvc.perform(post("/api/pickup-tokens/{orderId}/generate-qrcode", 4L))
                    .andExpect(status().isOk())
    ...
    void generateQRCode_filePathEncoding() throws Exception {
        ...
        Mockito.when(pickupTokenRepository.findByOrderId(5L))
                .thenReturn(Optional.of(token));
        ...
        try (MockedStatic<com.frh.backend.util.QRCodeGenerator> mocked =
            ...
            mockMvc.perform(post("/api/pickup-tokens/{orderId}/generate-qrcode", 5L))
                    .andExpect(status().isOk())
