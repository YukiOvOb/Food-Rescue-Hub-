...
class SupplierOrderControllerTest {
  ...
  void getOrderQueue_withoutStatus() throws Exception {
    ...
    Mockito.when(orderService.getOrderQueue(5L, null)).thenReturn(List.of(dto));

    mockMvc
        .perform(get("/api/supplier/orders/{storeId}", 5L))
  ...
  void getOrderQueue_withStatus() throws Exception {
    ...
    mockMvc
        .perform(get("/api/supplier/orders/{storeId}", 5L).param("status", "PENDING"))
        .andExpect(status().isOk());
  }
  ...
  void acceptOrder_success() throws Exception {
    ...
    Mockito.when(orderService.acceptOrder(20L)).thenReturn(order);

    mockMvc
        .perform(put("/api/supplier/orders/{orderId}/accept", 20L))
        .andExpect(status().isOk())
  ...
  void rejectOrder_success() throws Exception {
    ...
    Mockito.when(orderService.rejectOrder(30L, "Out of stock")).thenReturn(new Order());

    mockMvc
        .perform(
            put("/api/supplier/orders/{orderId}/reject", 30L)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isOk());
  }
  ...
  void cancelAcceptedOrder_success() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/supplier/orders/{orderId}/cancel", 40L)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isOk());
  }
