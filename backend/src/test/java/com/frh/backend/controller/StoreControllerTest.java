...
class StoreControllerTest {
  ...
  void createNewStore_success() throws Exception {
    ...
    Mockito.when(storeService.createStore(Mockito.any())).thenReturn(response);
  ...
  void getStoreById_success() throws Exception {
    ...
    Mockito.when(storeService.getStoreResponseById(2L)).thenReturn(response);

    mockMvc
        .perform(get("/api/stores/{storeId}", 2L))
  ...
  void getStoresBySupplier_success() throws Exception {
    ...
    Mockito.when(storeService.getAllStores(1L)).thenReturn(List.of(store));

    mockMvc
        .perform(get("/api/stores/supplier/{supplierId}", 1L))
        .andExpect(status().isOk())
  ...
  void updateStore_success() throws Exception {
    ...
    Mockito.when(storeService.updateStore(Mockito.eq(4L), Mockito.any())).thenReturn(response);

    mockMvc
        .perform(
            put("/api/stores/update/{storeId}", 4L)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isOk())
  ...
  void deleteStore_success() throws Exception {
    ...
    Mockito.doNothing().when(storeService).deleteStore(5L);

    mockMvc.perform(delete("/api/stores/delete/{storeId}", 5L)).andExpect(status().isNoContent());
  }
}
