...
class AnalyticsControllerTest {
  ...
  @MockitoBean private Co2AnalyticsService co2AnalyticsService;
  ...
  void getCo2Summary_success() throws Exception {
    ...
    summary.setCategories(
        List.of(
            new Co2CategoryBreakdownDto(
                1L, "Beef (beef herd)", new BigDecimal("2.000"), new BigDecimal("5.000"))));

    Mockito.when(co2AnalyticsService.getCo2Summary(supplierId, 30)).thenReturn(summary);

    mockMvc
        .perform(
            get("/api/analytics/supplier/{supplierId}/co2", supplierId)
                .param("days", "30")
                .accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
  ...
  void getCo2Summary_exception() throws Exception {
    ...
    mockMvc
        .perform(get("/api/analytics/supplier/{supplierId}/co2", supplierId).param("days", "30"))
        .andExpect(status().isInternalServerError())
