...
class ConsumerListingControllerTest {
  ...
  void getAllListings_success() throws Exception {
    ...
    Mockito.when(listingService.getAllActiveListings()).thenReturn(List.of(listing));
  ...
  void getListingsByCategory_success() throws Exception {
    ...
    Mockito.when(listingService.getListingsByCategory("Bakery")).thenReturn(List.of(listing));

    mockMvc
        .perform(get("/api/listings/category/{category}", "Bakery"))
        .andExpect(status().isOk())
