...
class CartControllerTest {
  ...
  void addItem_success() throws Exception {
    ...
    Mockito.when(
            cartService.addItem(Mockito.any(HttpSession.class), Mockito.eq(10L), Mockito.eq(2)))
  ...
  void updateQuantity_success() throws Exception {
    ...
    Mockito.when(
            cartService.updateQuantity(
                Mockito.any(HttpSession.class), Mockito.eq(20L), Mockito.eq(5)))
        .thenReturn(updatedCart);
  ...
  void removeItem_success() throws Exception {
    ...
    Mockito.when(cartService.removeItem(Mockito.any(HttpSession.class), Mockito.eq(30L)))
        .thenReturn(cart);

    mockMvc.perform(delete("/api/cart/items/{listingId}", 30L)).andExpect(status().isOk());
  }
  ...
  void clearCart_success() throws Exception {
    ...
    Mockito.when(cartService.clearCart(Mockito.any(HttpSession.class))).thenReturn(cart);

    mockMvc.perform(delete("/api/cart/items")).andExpect(status().isOk());
  }
}
