...
class ListingReviewControllerTest {
    ...
    void getReviewsByListing_success() throws Exception {
        ...
        Mockito.when(listingReviewService.getReviewsByListing(100L))
            .thenReturn(List.of(response));

        mockMvc.perform(get("/api/reviews/listing/{listingId}", 100L))
    ...
    void createReview_withConsumerSession_returnsCreated() throws Exception {
        ...
        Mockito.when(listingReviewService.createReview(10L, request))
            .thenReturn(response);
    ...
    void deleteReview_withConsumerSession_returnsOk() throws Exception {
        ...
        mockMvc.perform(delete("/api/reviews/{reviewId}", 9L).session(session))
            .andExpect(status().isOk());
