...
class AuthControllerTest {
  ...
  void register_consumer_success() throws Exception {
    ...
    AuthResponse response =
        new AuthResponse("token", 0L, "consumer@test.com", null, "CONSUMER", "Consumer registered");

    var consumer = Mockito.mock(ConsumerProfile.class);

    Mockito.when(authService.register(Mockito.any())).thenReturn(response);

    Mockito.when(authService.getConsumerByEmail(request.getEmail())).thenReturn(consumer);
  ...
  void register_supplier_success() throws Exception {
    ...
    AuthResponse response =
        new AuthResponse(
            "token", 10L, "supplier@test.com", null, "SUPPLIER", "Supplier registered");
    ...
    Mockito.when(authService.getSupplierByEmail(request.getEmail())).thenReturn(supplier);
