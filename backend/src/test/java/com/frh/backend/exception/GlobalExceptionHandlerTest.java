...
class GlobalExceptionHandlerTest {
  ...
  void handleRuntimeException_notFound() {
    ResponseEntity<Map<String, String>> response =
        handler.handleRuntimeException(new RuntimeException("Listing not found"));
  ...
  void handleRuntimeException_unauthorized() {
    ResponseEntity<Map<String, String>> response =
        handler.handleRuntimeException(new RuntimeException("Invalid email or password"));
  ...
  void handleRuntimeException_internalServerErrorFallback() {
    ResponseEntity<Map<String, String>> response =
        handler.handleRuntimeException(new RuntimeException("Unexpected"));
  ...
  void handleRuntimeException_forbidden() {
    ResponseEntity<Map<String, String>> response =
        handler.handleRuntimeException(new RuntimeException("Only consumers can checkout"));
  ...
  void handleRuntimeException_conflict() {
    ResponseEntity<Map<String, String>> response =
        handler.handleRuntimeException(new RuntimeException("Email already exists"));
    assertEquals(HttpStatus.CONFLICT, response.getStatusCode());
  }
  ...
  void handleRuntimeException_unauthorized_withNotAuthorisedMessage() {
    ResponseEntity<Map<String, String>> response =
        handler.handleRuntimeException(new RuntimeException("User not authorised"));
    assertEquals(HttpStatus.UNAUTHORIZED, response.getStatusCode());
  }
  ...
  void handleRuntimeException_conflict_withDuplicateMessage() {
    ResponseEntity<Map<String, String>> response =
        handler.handleRuntimeException(new RuntimeException("Duplicate account"));
    assertEquals(HttpStatus.CONFLICT, response.getStatusCode());
  }
  ...
  void handleBadRequest_withTypeMismatchException() {
    ...
    ResponseEntity<Map<String, String>> response =
        handler.handleBadRequest(conversionFailedException);
  ...
  void handleBadRequest_withMissingParameterException() {
    MissingServletRequestParameterException exception =
        new MissingServletRequestParameterException("storeId", "Long");
  ...
  void handleDataIntegrity_usesMostSpecificCauseMessage() {
    DataIntegrityViolationException exception =
        new DataIntegrityViolationException(
            "conflict", new RuntimeException("duplicate key value"));
  ...
  void handleDataIntegrity_fallsBackToExceptionMessageWhenCauseMissing() {
    DataIntegrityViolationException exception =
        new DataIntegrityViolationException("conflict fallback") {
          @Override
  ...
  void handleOptimisticFailure_returnsConflictBody() {
    ObjectOptimisticLockingFailureException ex =
        new ObjectOptimisticLockingFailureException(
            "Order", 1L, new OptimisticLockingFailureException("conflict"));
